...
//! is invoked with the live digest and mutated paths **after** they exist and
//! **before** [`decide_with_observations`](crate::harness_loop::decide_with_observations).
//!
...
pub trait CompletionAttestor: Send + Sync {
    ...
    fn attest(&self, digest: &str, paths: &[String]) -> Option<VerificationReport>;
}
...
pub fn merge_attested_report(
    ...
    let paths: Vec<String> = ledger.paths().map(str::to_string).collect();
...
mod tests {
    ...
    impl CompletionAttestor for SilentAttestor {
        fn attest(&self, _digest: &str, _paths: &[String]) -> Option<VerificationReport> {
            None
        }
    }
    ...
    fn empty_ledger_skips_attestor() {
        ...
        impl CompletionAttestor for PanicAttestor {
            fn attest(&self, _: &str, _: &[String]) -> Option<VerificationReport> {
