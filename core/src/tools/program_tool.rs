...
async fn run_quickjs_script(
    ...
    match result {
        Ok(Ok(result)) => {
            ...
            let rendered = render_script_output(&result, &records, "");
            let truncated = rendered.len() > max_output_bytes;
            let output = bound_program_output(rendered, max_output_bytes);
            let script_result = if truncated {
...
async fn run_embedded_script(
    ...
    let result_json = async_with!(context => |ctx| {
        ...
        // Catch the thrown value. `Error::Exception` alone prints as
        // "Exception generated by QuickJS" and drops the interrupt message
        // ("interrupted"), so a deadline would look like a script bug.
        let promise: Promise = match ctx.eval(script).catch(&ctx) {
            Ok(promise) => promise,
...
fn bound_program_output(output: String, max_output_bytes: usize) -> String {
    const MARKER: &str = "\n[output truncated]";
    if output.len() <= max_output_bytes {
        return output;
    }
    let budget = max_output_bytes.saturating_sub(MARKER.len());
    format!("{}{MARKER}", truncate_utf8(&output, budget))
}
