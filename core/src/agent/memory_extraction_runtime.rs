...
impl AgentLoop {
    pub(super) async fn schedule_turn_memory_extraction(
        ...
        if event_tx.is_some() {
            ...
            let _ = (cancel_token, task_context);
            // Streaming hosts cancel the turn as soon as `End` is emitted. Memory
            // extraction must outlive that turn cancel; session close still drains
            // these tasks through AgentMemory. Capability-run promotion remains
            // best-effort via the dedicated scoped test path in unit coverage of
            // AgentCapabilityRuntime, but production streaming must not inherit
            // the turn token or extraction never reaches the model.
