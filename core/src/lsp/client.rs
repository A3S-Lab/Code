...
mod tests {
    ...
    async fn test_lsp_client_spawn_with_args() {
        ...
        let result = LspClient::spawn("test".to_string(), "cat", &args, &HashMap::new()).await;
    ...
    async fn test_lsp_client_spawn_with_env() {
        ...
        let result = LspClient::spawn("test".to_string(), "cat", &[], &env).await;
        if let Ok(client) = result {
            let _ = client.close().await;
        }
    }
    ...
    async fn test_lsp_client_is_connected() {
        let result = LspClient::spawn("test".to_string(), "cat", &[], &HashMap::new()).await;
        if let Ok(client) = result {
            assert!(client.is_connected());
