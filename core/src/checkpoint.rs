    ...
    pub async fn diff(
        ...
        for (rel_path, content) in &checkpoint.files {
            ...
            if current != *content {
                diffs.push(FileDiff {
                    path: rel_path.clone(),
                    diff: generate_diff(&current, content, rel_path),
                });
            }
        }
        Ok(diffs)
...
mod tests {
    ...
    async fn test_checkpoint_restore_files() {
        ...
        tokio::fs::write(workspace.path().join("test.rs"), "old content")
            .await
            .unwrap();
        ...
        let content = tokio::fs::read_to_string(workspace.path().join("test.rs"))
            .await
            .unwrap();
    ...
    async fn test_checkpoint_diff() {
        ...
        tokio::fs::write(workspace.path().join("test.rs"), "current")
            .await
            .unwrap();
