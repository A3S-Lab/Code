...
mod extra_reflection_tests {
    ...
    fn test_suggested_actions() {
        assert!(ErrorCategory::SyntaxError
            .suggested_action()
            .contains("syntax"));
        assert!(ErrorCategory::NotFound.suggested_action().contains("path"));
        assert!(ErrorCategory::PermissionDenied
            .suggested_action()
            .contains("permission"));
        ...
        assert!(ErrorCategory::Timeout
            .suggested_action()
            .contains("timeout"));
        assert!(ErrorCategory::InvalidArguments
            .suggested_action()
            .contains("argument"));
        assert!(ErrorCategory::AlreadyExists
            .suggested_action()
            .contains("name"));
        assert!(ErrorCategory::MissingDependency
            .suggested_action()
            .contains("dependency"));
        assert!(ErrorCategory::RuntimeError
            .suggested_action()
            .contains("runtime"));
    ...
    fn test_strategy_selector_forced_overrides_prompt() {
        let selector = StrategySelector::new().with_forced_strategy(ExecutionStrategy::Direct);
