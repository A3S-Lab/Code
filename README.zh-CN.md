...
**A3S Code** 是一个异步 Rust 运行时，用于构建受治理的编码代理。它
保持代理循环、工作区工具、模型适配器、策略决策、
版本化事件、工作区检索和背后的持久证据
明确的合同。从 Rust、Node.js、Python、Go 或通过
`a3s code`终端应用。

<p align="center">
  <a href="#start-in-60-seconds">开始</a> ·
  <a href="#whats-new-in-83">v8.3</a> ·
  <a href="#whats-new-in-80">v8.0</a> ·
  <a href="#why-a3s-code">为什么要编码</a> ·
  <a href="#capability-map">功能</a> ·
  <a href="#configure-the-runtime">配置</a> ·
...
- **可协商的会话存储持久性 (KRN-6)。** 聚合 CAS，仅附加
  WAL、写入器租赁防护、可选 AES-256-GCM 静态加密、提交
  监视和参考感知工件 GC 仅在经过验证后才会进行广告。
- **键入工具结果信任 (KRN-5)。** 受信任/工作区/外部标签
  穿过工具→模型边界；运行绑定调用之前承认提示信任
  后来的中间件； `model_middleware_health` 在所有 SDK 上都是无秘密的。
- **工作区源快照 (KRN-4)** 将检索结果绑定到
  在派生索引重建后仍然存在的防篡改身份。
- **易出错的 FFI 运行时 init (KRN-9)**，适用于 Node.js 和 Python，以及主机拥有的
  不可变内容适配器、检查点导出接收器和技能功能
  Node.js / Python / Go 上的批处理。
- **Linux arm64 Python 轮子** 作为 `manylinux_2_39_aarch64` (glibc 2.39+) 发布
  以匹配捆绑的 zvec 运行时。
...
- **运行拥有的时空组合。** 会话、运行、轮次和子任务范围
  现在形成一棵仅向下的权威和取消树，具有有界，
  逆序效应结算。
- **一代精确的能力预测。**工具、技能、代理、命令、
  Hooks、MCP、Context、Flow、Knowledge 和 UI 值以原子方式发布，
  在每次承认的跑步的一生中保持固定。
- **精确的时间恢复。** 运行和逻辑检查点证据绑定
  代码目录、完整的权限上限、可选的A3S使用光标。安尼
  检查点无法通过 N+1 静默恢复。
- **便携式检查点工件。** 规范语义和逻辑状态是
  内容寻址为一个主机可存储的有效负载，并具有故障关闭漂移检查
  以及新鲜会话精确的历史引导路径。
- **有界模型证据。** 工具请求、确定性结果转换、
  不可变的原始内容引用、模型输入和功能表面
  是摘要绑定的，不保留凭据或提示明文。
- **收敛工作流程结果。** 可恢复的工作流程检查点和流程
  决策声明携带与规范相关的有界的、仅摘要的结果收据
  执行身份；陈旧或不可读的状态无法关闭，而遗留
  记录保持可加载。
- **收敛工作流程准入。**动态流程步骤项目到同一流程中
  `ExecutionPlan` 由代码规划使用，从完整的重建该计划
  恢复历史记录，并使用可取消的每个工作流并发门。
  独立流适配器还可以使用代理范围的优先级
  具有仅摘要步骤标识和所有者配额的调度程序；会话限制
  调用保留一个外部调度程序租约以避免嵌套单槽
  僵局。分离的子级继承运行/会话准入范围。
- **提供者感知的一代入场。** 定期、流式和结构化
  模型调用在会话之间共享一种类型的提供者/模型容量身份，
  委派子项、直接工具和动态工作流程。叶世代
  通过现有的调度程序参与者保留该容量，而无需创建
  第二个队列；取消和丢弃的流释放本地和共享
  自动预订。 Rust 主机可以无秘密地检查
  `ModelGenerationPoolHealthSnapshot`；调度程序只保留有界的
  已完成的池周期的最近运行状况窗口。
- **一代围栏工作流程重播。**新的动态工作流程运行固定
  代码运行时构建并公开派生的仅摘要延续身份
  来自持久不变的事实。更改源/输入，步骤冲突
  定义和不支持的运行时生成在步骤之前被拒绝
  执行，而遗留的未固定历史在期间仍然可读
  迁移。现在，稳定的索赔身份将每个工人与当地的
  文件支持（或主机注入）租约，心跳使实时所有者受到隔离，
  过时的工作人员在工作流程/步骤准入之前被拒绝，并且父级工作人员
  取消解决或保留租约，而不是释放
  飞行中的工作人员。主机可以绑定一个`DynamicWorkflowControl`句柄到
  检查有限的仅摘要快照，读取可信历史记录，驱动运行，
  或请求/强制永久取消；本地期刊使用跨流程
  锁定，而 Flow 保留唯一的事件权限。

Go 消费者必须将模块路径更新为
`github.com/A3S-Lab/Code/sdk/go/v8`。参见[CHANGELOG.md](CHANGELOG.md)
完整的兼容性和发布记录。
...
|面积 |有什么可用 |激活|
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
|代理运行时 |异步`Agent`、工作区限制`AgentSession`、发送、流式传输、恢复、替换、取消、关闭、重播和安全点`steer`/`interrupt`运行控制 |基线|
|受控工具 |文件、搜索、shell、Git、Web、结构化生成、批处理、程序、技能、MCP、委托、确定性结果投影和证据 |仅在工作空间和政策允许时才公开 |
|评估基材|提供者中立的执行目标/框架、仅摘要的事实日志、原子有界证据快照、独立的辅助运行、主机边界监督、重新启动安全调度租约、持久结果 CAS 以及严格版本化的 Rust/Node/Python/Go 线投影 |注入 `EvaluationPolicy`/`AuxiliaryExecutor` 以及可选的调度/结果存储；核心供应机制和生成的传输模式，而审阅者规则、调查结果、授权和云审计仍然由主机拥有 |
|本土研究合同|版本化的摘要绑定研究运行、证据事实、声明、引文、具有出版完整性的证据图、具有结果收据绑定的工作流程计划、发现触发的重新运行谱系、再现性清单、出处收据、审查结果、项目事件以及具有生成的 Node/Python/Go 投影、有界字段和故障关闭生命周期转换的版本化研究线信封 |主机绑定精确的源/证据快照和`RunCapabilityBindingV1`； A3S Use提供包/环境身份和桌面/云自己的科学政策、审查决策、保留和发布 |
...
|认知包 | Exact A3S Use生成绑定、主机注入引用 Markdown 提供程序、有界源验证、重新启动检查和失败关闭检索 | Rust 主机注入`CognitiveContextSession`；代码从不安装或解析包 |
| A3S Use运行时任务 |通过主机拥有的调度程序进行精确的功能快照 v2 运行时工具投影和模型可见的受控调用 |原子使用支持的`SessionCapabilityBatch`中的阶段`UseRuntimeTaskProjectionAdapter`；代码从不启动预计的命令或直接获取包状态 |
|型号适配器| Anthropic、Zhipu、OpenAI 兼容 API 和自定义 `LlmClient` 实现；每个运行绑定调用都会通过一个显式中间件管道（信任→预算→证据→生成→提供者→使用）|配置或主机注入；外部工具结果需要在及时使用之前进行编辑审查 |
...
|优先调度|跨会话、直接工具、分离的后台子进程和主机工作流程的代理范围`a3s-lane`优先级/先进先出准入，具有取消、饥饿安全老化、仅摘要所有者/提供者配额、仅配额叶预留、占用快照和有界累积运行状况计数器 |基线；调整`task_scheduler`，选择每个会话`TaskPriority`，检查`task_scheduler_stats()`或`task_scheduler_health()`；主机可以使用 `TaskSchedulerQuota` 进行范围限制，使用 `model_generation_pool_health()` 进行会话的提供程序池，以及 `model_middleware_health()` / SDK 等效项进行无秘密中间件阶段计数器 |
...
|可编程工作流程|有界 QuickJS `program` 调用、可重播的 A3S Flow 支持的动态工作流程、可恢复的步骤检查点和摘要绑定结果收据 | `program`基线；动态运行时显式注册|
|坚持|原子快照，具有文件存储意图/提交的 WAL 恢复、聚合 CAS (`save_snapshot_cas`)、编写者租赁防护 (`acquire_writer_lease`)、提交监视 (`watch_commits`)、可选 AES-GCM 静态加密 (`with_encryption_key`)、通过 SessionStore 往返的引用感知工件保留 (`reference_aware_artifact_gc`)、运行事件、跟踪、工件、验证、身份绑定工作流程/流程收据、检查点和可选的 RL 轨迹 |配置存储和主机策略；在依赖仅附加 WAL、聚合 CAS、租赁防护、监视、静态加密、引用感知 GC 或其他 KRN-6 保证之前协商`SessionStoreCapabilities`； GC 之前固定工件 URI |
|状态图|哈希链接事件、类型化对象和关系、乐观补丁、严格重放、分叉、差异和 Flow 0.11 生命周期预测，包括取消、最终结果、进度和子操作 |显式应用程序使用 |
|代理解除合同|有界 `.a3s/asset.acl` 准入、规范身份、出处绑定和兼容性检查 |基线准入API |
...
需要显式 zvec-grep-style 重启持久性的主机可以使用
...
具有有界重叠，以及 Rust 主机提供的自定义范围分割器。代码
...
主机注入的进程内 CPU 回调；它不需要是远程的或使用
图形处理器。可选的确定性 MMR 重新排序器也是无模型的 CPU 代码。

主机可以实现公共`EmbeddingProvider`特征，而无需添加
...
Node 和 Python 主机通过传递类型来选择加入
`DeterministicWorkspaceReranker`至`WorkspaceRetrievalOptions`；去分配
`NewDeterministicWorkspaceReranker()` 到键入的 `Reranker` 字段。省略
...
这些三任务奇偶校验运行不符合新的默认值。
2026 年 8 月 17 日以代码 `5aa9642` 发布后的 `v7.0.1` 重播重复了所有九个
...
它们为 5/2/3，因此模型选择是主机拥有的，既不是模型也不是
...
网络证据保留了键入的重试决策。 `web_search`记录等级品质，
...
Rust 主机可以在现有拥有的内存中安装`ScheduledSemanticRefresh`
...
启用Code的`durable-memory-sqlite`功能的Rust主机可以注入A3S
...
主机可以通过以下方式提供类型化的`Arc<dyn FlowEventStore>`
...
通过 `EventEnvelopeV1` 重播投影这些值，从而保留其
...
几代人。运行公开状态、活动工具、有序事件重播、独占
分页光标和保留间隙检测。文件持久化使用原子性
更换；工件受项目计数和字节的限制；验证保留
主张与证据分开。

Rust 主机可以映射一个完整的快照和一个可选的精确工具间映射
...
该合同用于协商可选功能。 Node.js、Python 和 Go 主机可以注入类型化异步嵌入
...
主动重播。除非有明确的受控工具适配器，否则流将保持仅主机状态
...
`serve` 功能，主机可以提供代理目录并运行 cron 计划。
可观察的守护进程句柄仅在计划、会话和
...
| [Advanced Developer Manual](manual/ADVANCED_DEVELOPER_MANUAL.md) · [Chinese](manual/ADVANCED_DEVELOPER_MANUAL_CN.md) |扩展合同、安全性、生命周期和生产集成 |
...
| [Harness Model-Call Evidence](manual/HARNESS_EVIDENCE.md) |功能/输入/使用快照、重复上下文诊断、事件排序、编辑边界、验证和重播 |
