...
事实日志是唯一的控制源。下一步转移只由折叠这条不可变日志决定（`a3s-effect`
的 `ingest` / `resume`）。`send`、`stream`、附件回合和精确恢复都向这条日志
追加，或从它恢复。已经存下的 `model.turn` 不会再次发给模型。循环检查点不决定
下一次模型调用。确认停在 `confirmation.answered` 之前。提问停在
`question.answered` 之前，重新打开的日志仍带着 `allow_free_text` 和宿主要渲染
的选项。确认和提问不会由进程内的 oneshot 或计时器了结。转向是另一条
`user.message`。缺失的工具结果在恢复时只跑一次。工具回合上限是一次空工具列表
的补全。工作区工具、模型适配器、上下文构造和会话快照仍留在 Code。

<p align="center">
  <a href="#60-秒内起步">起步</a> ·
  <a href="#90-有什么新内容">v9.0</a> ·
