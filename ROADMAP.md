...
| `TB-RETRY1` | In progress | Core now promotes retry exhaustion to a typed terminal Agent error and retains bounded status/body diagnostics; Provider retry, stream-restart, and Agent circuit-breaker ownership is being consolidated | A dedicated call-count regression proves 429/5xx `Retry-After` is honored once, inner exhaustion is not replayed by an outer fallback, and cancellation/deadline bounds every backoff |
