...
| `WSR-PROD1` | In progress | Code `beac7cb` adds the first production-hardening slice: a revision-locked real embedding model matrix through the Python SDK. English MiniLM is a language-boundary negative control at Recall@5 `0.6667`; multilingual MiniLM reaches semantic and RRF-hybrid Recall@5 `1.0`, MRR `0.5`, ranks `2/2/2`, 1.0x request amplification, zero non-text inputs, and complete release. Deterministic reranking retains Recall@5 but lowers MRR to `0.3444`, validating RRF-only as the production candidate. Real CLI HTTP-provider execution, representative generation tasks, soak/churn, cross-platform evidence, and operational SLO closure remain required |
