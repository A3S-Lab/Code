...
| `TB-RETRY1` | In progress | Core now promotes retry exhaustion to a typed terminal Agent error, retains bounded status/body diagnostics, and makes streaming Provider backoff cancellation-aware; Provider retry, stream-restart, and Agent circuit-breaker ownership is being consolidated | A dedicated Agent call-count regression must still prove 429/5xx `Retry-After` is honored once and inner exhaustion is not replayed by an outer fallback; cancellation-aware backoff coverage is delivered, while the Agent deadline gate remains with the in-flight boundary work |
