...
| --- | --- | --- | --- |
| `CAR-01` | In progress | Conform the native Harness to Cloud `A1.2`/`A1.3` command, receipt, event-page, cancellation, recovery, fail-closed workspace/session admission, bounded change-set capture-state, and balanced external-task lifecycle contracts | Cloud retains execution identity and sequencing authority |
...
| `TB-RETRY1` | In progress | Core now promotes retry exhaustion to a typed terminal Agent error, exposes bounded status/body diagnostics, makes streaming Provider backoff cancellation-aware, and has an end-to-end call-count regression for inner exhaustion versus the Agent circuit breaker | The 429/5xx inner-budget, typed classification, and cancellation-backoff checks are delivered; the Agent deadline/cancellation gate and its committed in-flight timeout regression must still settle before this gate is marked Delivered |
