...
| `RESEARCH-EXEC1` | In progress | Host adapter that drives a research run through source capture, evidence append, artifact publication, and evaluator dispatch while retaining one Code Run identity; Code now exposes exact execution-target validation and explicit Run-aware Core-event projection | Integration tests must still prove restart, cancellation, contiguous evidence, artifact immutability, and exact Code/Use binding across a real workspace |
