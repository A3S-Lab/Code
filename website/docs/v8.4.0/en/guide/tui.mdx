...
| PTC scripts       | The `program` tool runs sandboxed JavaScript-compatible scripts with a host-provided `ctx` object. Recursive `program`, `dynamic_workflow`, and the removed `parallel_task` alias stay out of the default allow-list; one-item `task` calls are allowed, but direct fan-out is blocked. |
...
implementation outside QuickJS. If A3S OS registers a runtime tool after
`/login`, dynamic workflow PTC steps may also call `ctx.tool("runtime", ...)`.
