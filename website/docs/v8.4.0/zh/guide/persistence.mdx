...
`SessionStoreCapabilities` 现在也会声明可选的 KRN-6 保证。内置适配器仅在真正
兑现语义时置位；宿主必须先协商再依赖：
...
| `lease_fencing`               | `acquire_writer_lease` 发布耐久 epoch；接管后旧持有者在提交时 fail-closed。             |
...
即使启用静态加密，仅含 digest 的 WAL 也保持未加密。memory / file 适配器继续声明
它们已经兑现的原子快照；新标志默认不声明，直到完成配置。
