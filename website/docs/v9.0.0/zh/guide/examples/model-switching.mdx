...
async fn main() -> a3s_code_core::Result<()> {
    ...
    let scout = WorkerAgentSpec::read_only("scout", "读取文件并报告发现。")
...
- 请在 worker spec 上设置覆盖。`task` 工具只接受 `agent`、`description`、`prompt`、
  `background`、`max_steps` 和 `output_schema`，其他字段（包括 `model`）都会被拒绝。
