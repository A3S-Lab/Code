...
已验证的本地工作区 `toolNames()` 集合包含 `read`、`write`、`edit`、`patch`、
`download`、`search`、`ls`、`bash`、`task`、`search_skills`、`Skill`、
`program`、`git`、`batch`、`web_fetch` 和 `web_search`。

直接工具调用是宿主侧特权能力。把它暴露给最终用户之前，应在宿主应用内做权限判断。

`download` 通过通用直接工具 API 调用：
...
8 GiB，`timeout` 默认 300 秒、最大 3600 秒，`expected_sha256` 必须正好包含
64 位十六进制字符。`file_path` 是工作区相对路径，也可以省略并安全推断文件名；
`overwrite` 默认 `false`。

该工具只存在于可写本地工作区。模型驱动调用仍是受权限与 HITL 治理的工作区修改。
传输会为请求保留签名查询参数，但从结果元数据中移除它们；逐跳执行 SSRF 与直接 DNS
目标校验，严格验证 Range 响应，只做有限重试，必要时顺序回退，并且只有完整下载和
可选摘要校验成功后才把相邻临时文件提升为目标。
...
Headless 宿主可以准入精确且不可变的运行标识，并立即取得权威快照，而无需等待运行结束：
...
重复提交兼容的不可变输入会返回 `replayed: true`，不会启动重复工作。输入冲突会返回
`RUN_IDENTITY_CONFLICT`；关闭会话会取消已分离的后台 worker。
...
> 完整指南：[持久化](/guide/persistence)与[会话](/guide/sessions)。
...
带版本号的 `SessionSnapshotV1`。文件或内存存储会原子发布这个聚合快照。较早的分片
记录仍可加载；自定义存储必须显式实现聚合保存。工作区 `.a3s/effect-log/` 下的事实
日志是编码回合的控制来源，不属于快照。
...
> 完整指南：[任务](/guide/tasks)与[编排](/guide/orchestration)。
...
两个辅助方法都返回来自模型可见 `task` 工具的 `ToolResult`。
...
> 完整指南：[MCP](/guide/mcp)。闲置断开见[集群扩展点](/guide/cluster-extension-points)。
...
配置了 `SessionStore` 后，智能体循环会在每个写入事实日志的工具结果之后持久化一个
`LoopCheckpoint`（按 `run_id` 索引）。任何拥有同一个存储的节点都能恢复该运行：检查点
为一个全新的事实日志线程播种，再由折叠该日志选择下一步。检查点本身从不选择下一次
模型调用。
...
恢复的工作会记录为一个**新**运行；检查点所属的运行不被修改。没有检查点时，如果会话
已有事实日志，`resumeRun` 会折叠该日志（静止的日志不执行任何步骤）。只有在日志也为空
时，才会出现下面两种错误：

- `"resume_run requires a session_store on this session"`：宿主应该回退到新建会话。
- `"no loop checkpoint found for run 'X'"`：该运行从未写出检查点，或检查点已在运行
  结束时删除。

日志中缺少结果的工具调用会在恢复时执行一次，因此宿主应让非幂等工具经过确认或使用
幂等键。需要精确、可安全重放的恢复标识时，使用
...
guard 对象上缺失的方法会使用宽松默认值（Allow / no-op）。Python 与 Go 回调报错、
超时或返回无法解析的决策时会 fail-closed 为 Deny。
...
异常会 fail-closed。Go handler 接收带类型的 context，报错或超时会 fail-closed。

清除时，Node 向 `session.setBudgetGuard` 传入 `null`；Python 调用
