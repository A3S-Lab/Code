---
title: 'skills/ 技能目录'
description: '用工作区 skills/ 组织可复用技能和检查清单。'
---
...
Skill 目录永远不会被隐式扫描。通过 ACL `skill_dirs`（该 agent 的所有 session 共享）
或 session `skillDirs` 加载它们。Worker 定义使用 `agentDirs`，不要把 skill 目录放进
`agentDirs`。
...
每个目录都会被递归遍历。skill 可以是 `*.md` 文件，也可以是子目录中的 `SKILL.md`，
因此两种布局都可用：
...
不存在的目录加载 0 个 skill。两个文件声明相同 `name` 时，后加载的会替换先加载的，
并记录 warning。
