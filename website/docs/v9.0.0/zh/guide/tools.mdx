...
| 代码智能                   | `code_symbols`、`code_navigation`、`code_diagnostics`                       | 仅当工作区携带代码智能提供者（`WorkspaceServicesBuilder::code_intelligence`）时注册。默认本地后端没有提供者。                                 |
| 运行时内置工具             | `web_fetch`、`web_search`、`update_plan`、`ask_user`、`batch`、`program`    | 由核心工具执行器注册。`batch` 和 `program` 接收当前有作用域的调用器，因此内部工具既受会话表面限制，也继承调用方的治理作用域。                 |
| 会话启动工具               | `task`、`generate_object`、`search_skills`、`Skill`                         | 构建 `AgentSession` 时加入。`task` 是唯一模型可见的委派 schema；多条目扇出通过 `task` 完成。关闭手动委派后会移除委派能力。                    |
...
呈现配置（`a3s.code.tool-presentation-profile.v1`，`core/src/tools/presentation.rs`）
决定模型在某个提示词下看到哪些工具定义。它只改变呈现；权限策略、确认和沙箱仍然治理
每一次调用。

| 模式       | 面向模型的定义                                            |
| ---------- | --------------------------------------------------------- |
| `adaptive` | 默认值。按提示词选择相关的子集。                          |
| `direct`   | 权限可见性允许的全部定义。                                |
| `code`     | 把 `program` 作为通往其他工具的紧凑代码网关。             |
| `disabled` | 不提供工具。`session.tool(...)` 等宿主直连 API 不受影响。 |
...
| 运行时内部调用       | `dynamic_workflow` 启动私有 `program` 执行引擎                                              | 只跳过已经由 `dynamic_workflow` 完成的重复权限与人工确认；Hook、证据、预算、取消、递归检查和脚本中的实际工具仍受治理。 |
...
当 `read` 打开 JPEG、PNG、GIF 或 WebP 文件时，工具返回图片 `Attachment`，而不是把
字节当作文本。这与工具描述一致。在 OpenAI 兼容提供方上，工具结果消息会保留多模态
`image_url`，而不是把这些附件压成占位文本（#156 / #152）。
...
`grep`，路径发现使用 `glob`，原生词汇相关性排序使用 `bm25`。这些模式是分离的平面：
`grep` 不会打开持久 a3s-vec FTS，`bm25` 也不会成为精确匹配权威。宿主显式启用 Workspace
...
内置工作区后端在非内容模式下不会构造随后被丢弃的匹配文本。在默认 `local-code`
下，清单支持的工作区还会在 `.a3s-code/grep-trigram` 下惰性构建进程内 trigram
候选缓存，使字面量模式在精确正则扫描前打开更少文件。非字面量模式与索引失败会失败
开放回今日的全量扫描。S3 结果会设置
...
已配置的原生 API 引擎；只有合并结果未达到结构化检索要求时，才运行普通 HTTP/RSS
引擎；前两层仍不足时，才运行无头浏览器引擎
（`core/src/tools/builtin/web_search/fallback.rs`）。`cascade_order` 可以覆盖该顺序。
无头层只存在于启用 `headless-search` 特性的构建中（`scientific` 和 `full` 包含该特性，
Core 默认特性不包含），浏览器发现和池创建都是惰性的。基于 Moli 的无头引擎随
`a3s-search` v3.1.0 引入；A3S Code 9.0.0 锁定 `a3s-search` 3.1.4，并在运行无头层时使用
打包或共享缓存中的 Moli Runtime。Chrome 和 Lightpanda 仍需显式配置。
...
结构门槛只决定级联是否继续下一层。默认顺序是 API，然后 HTTP/RSS，最后才是
headless。有可用结果就是成功的工具结果：门槛达到时为 `complete`，未达到时为
`partial`。两种情况下 JSON 都保持结果数组契约。空结果、未知引擎和非法参数仍然是
错误。Search 不依赖外部语义验证器或重排序 API。
...
`web_fetch`、`download` 和 `web_search` 按以下顺序选择一个代理：`search.headless.proxy_url`
配置，然后是 `HTTPS_PROXY`、`https_proxy`、`HTTP_PROXY` 或 `http_proxy`，最后是 macOS
系统代理。`web_search` 单次调用的 `proxy` 参数优先于以上全部来源。选中代理后，这些工具
的每个请求都经过该代理，且不会参考 `NO_PROXY`（`core/src/tools/builtin/safe_http.rs`）；
如果某个目标必须直连，请为该进程取消代理变量。没有代理时，这些工具直接连接经过校验的
公网地址。`core/src/llm/http.rs` 中的 `NO_PROXY` 处理适用于模型 provider、MCP HTTP 和
OAuth 客户端，不适用于网页工具。
...
`ctx.readFile(path, options)` 返回选定文本，不包含 `read` 工具的行号锚点和续传页脚，
适合直接进行字符串处理。脚本需要完整工具结果、带行号的 `output`、退出码、元数据和
续传证据时，应使用 `ctx.read(path, options)`。两种形式都会调用同一条受治理的 `read`
工具，并消耗相同的脚本调用预算。
...
不会允许递归调用 `program` 和 `dynamic_workflow`。
QuickJS 可以用单个 `tasks` 项调用 `task`，但会阻止直接多任务扇出。动态工作流需要本地
并行子智能体时，会调度名为 `task` 的 Flow 步骤，由 TUI 宿主在 QuickJS 外执行。完成
`/login` 且会话中已经注册宿主运行时工具后，动态工作流的
PTC 步骤还可以调用 `ctx.tool("runtime", ...)`。

模型选择的 `dynamic_workflow` 获得授权后，其私有 QuickJS 执行引擎不需要额外的
`program(*)` 规则。`allowed_tools` 中列出的每个实际工具仍会重新经过正常的权限、
确认、Hook、预算、沙箱与取消路径。
...
`session.verifyCommands(...)` 返回的报告是宿主证据。它没有绑定到回合的 mutation
effect digest，因此不能关闭完成门禁；见[验证](/guide/verification#完成门禁)。
