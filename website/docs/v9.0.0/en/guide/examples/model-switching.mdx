...
async fn main() -> a3s_code_core::Result<()> {
    ...
    let scout = WorkerAgentSpec::read_only(
        "scout",
        "Reads files and reports findings.",
    )
        .with_model_ref("provider/fast-model");
    let session = agent
        .session_builder("/repo")
        .options(
            SessionOptions::new()
...
func main() {
	...
	session, err := agent.Session(ctx, "/repo", &code.SessionOptions{
		...
		Model: "provider/review-model",
		// High-volume exploration runs on the cheaper model.
		WorkerAgents: []code.WorkerAgentSpec{{
			Name:        "scout",
			Description: "Reads files and reports findings.",
			Kind:        "read_only",
			Model:       "provider/fast-model",
...
- Set the override on the worker spec. The `task` tool accepts only `agent`,
  `description`, `prompt`, `background`, `max_steps`, and `output_schema`, and
  rejects any other field, including `model`.
