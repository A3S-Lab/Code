...
// 2. Search the workspace for the symbol we care about (returns the grep tool's text).
...
`glob` returns a list of matching file paths, while `grep` returns the
`grep` tool's text output as a single string. Both run locally and return quickly, so you
...
rather than just the matching lines; it returns the `read` tool's line-numbered view.
