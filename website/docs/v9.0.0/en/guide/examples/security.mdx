...
async fn main() -> a3s_code_core::Result<()> {
    ...
    let permission_policy = PermissionPolicy::new()
        ...
        .ask("bash(*)");
    let confirmation_policy = ConfirmationPolicy::enabled();
...
def main() -> None:
    ...
    # Turn the `ask` patterns into a human-in-the-loop confirmation flow.
    opts.confirmation_policy = ConfirmationPolicy(enabled=True)
...
func main() {
	...
	session, err := agent.Session(
		...
		&code.SessionOptions{
			...
			ConfirmationPolicy: &code.ConfirmationPolicy{Enabled: code.Ptr(true)},
		},
	)
	if err != nil {
		log.Fatal(err)
...
- A `confirmationPolicy` with `enabled: true` is what turns `ask` decisions into a
  ...
  `session.confirmToolUse(toolId, approved, reason?)`. A confirmation parks the run
  until that answer (or a cancellation) arrives; `defaultTimeoutMs` and
  `timeoutAction` do not settle it. Without an enabled confirmation policy, an
  `ask` decision runs the tool.
