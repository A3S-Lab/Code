...
func main() {
	...
	session, err := agent.Session(ctx, "/repo", &code.SessionOptions{
		PromptSlots: &code.PromptSlots{
			Role: "release-readiness reviewer",
			Guidelines: "Find blockers before improvements. " +
	...
	result, err := session.Run(ctx, "Is this repo ready to ship?")
	if err != nil {
		log.Fatal(err)
	}
...
async fn main() -> a3s_code_core::Result<()> {
    ...
    let role_only = agent
        ...
        .options(SessionOptions::new().with_prompt_slots(
            SystemPromptSlots::default().with_role(
                "You are a senior Rust developer who specializes in async programming.",
            ),
        ))
        .build()
        .await?;
    ...
    let reviewer = agent
        ...
        .options(SessionOptions::new().with_prompt_slots(
            SystemPromptSlots::default()
                .with_role("You are a Python code reviewer.")
                .with_guidelines("Always check for type hints. Flag any use of `eval()`.")
                .with_response_style("Reply in bullet points. Be concise."),
        ))
        .build()
        .await?;
    ...
    let extra_only = agent
        .session_builder("/repo")
        .options(SessionOptions::new().with_prompt_slots(
            SystemPromptSlots::default().with_extra("Always end your response with '-- A3S'"),
        ))
        .build()
        .await?;
    ...
    let file_manager = agent
        ...
        .options(SessionOptions::new().with_prompt_slots(
            SystemPromptSlots::default()
                .with_role("You are a minimalist file manager.")
                .with_guidelines("Only create files when explicitly asked."),
        ))
        .build()
        .await?;
    let result = file_manager
        .send(
            "Create test.txt with 'prompt slots work', then read it back.",
            None,
        )
        .await?;
    println!("{}", result.text);
