...
`MemoryRecent`, and the working/short-term memory methods. `rememberSuccess` takes
a short task description, the tools involved (also used as tags), and the
outcome text; `rememberFailure` takes the task, the error text, and then the
tools. If the default file store cannot be created, the session falls
