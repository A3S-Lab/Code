...
and `Skill` (invoke a skill by name). `Skill` runs a child agent loop that uses the
skill's content as its role prompt and returns that run's final text. You can let the model
...
A `SKILL.md` declares its `kind` in frontmatter (`instruction`, the default, `persona`,
or `tool`). The `Skill` tool grants the skill's `allowed-tools` only for the child run,
composed with the session's permission policy, and revokes them afterwards. A skill
that disables model invocation cannot be run through `Skill`. Skill
