...
async fn main() -> a3s_code_core::Result<()> {
    ...
    let allowed = HashSet::from(["glob", "grep", "read", "generate_object"]);
...
`readFile` / `read_file` / `ReadFile` returns the `read` tool's view: each line is
prefixed with a right-aligned line number and a tab, and long files end with a
continuation hint. Strip the prefix before parsing file content.
