...
`patch`, `download`, `search`, `ls`, `bash`, `task`, `search_skills`, `Skill`,
...
`download` is invoked through the generic direct-tool API:
...
The tool exists only for writable local workspaces. Model-driven calls remain
permission- and HITL-governed workspace mutations. The transfer preserves
signed query parameters for requests but removes them from result metadata,
validates every redirect and direct DNS target against SSRF, verifies strict
Range responses, retries only within fixed bounds, falls back sequentially
when needed, and promotes an adjacent temporary file only after completion and
...
> Full guide: [Verification](/guide/verification).
...
> Full guide: [Memory](/guide/memory).
...
> Full guide: [Skills](/guide/skills).
...
> Full guide: [Sessions](/guide/sessions).
...
Headless hosts can admit exact immutable run identities and receive the
authoritative snapshot without waiting for completion:
...
duplicate work. Conflicting input returns `RUN_IDENTITY_CONFLICT`; closing the
session cancels detached workers.
...
> Full guide: [Persistence](/guide/persistence) and [Sessions](/guide/sessions).
...
> Full guide: [Tasks](/guide/tasks) and [Orchestration](/guide/orchestration).
...
Both helpers return `ToolResult` values from the model-visible `task` tool.

## Hooks

> Full guide: [Hooks](/guide/hooks).
...
> Full guide: [Commands](/guide/commands).
...
> Full guide: [Lane Queue](/guide/lane-queue).
...
> Full guide: [MCP](/guide/mcp). Idle disconnect: [Cluster Extension Points](/guide/cluster-extension-points).
...
> Full guide: [Cluster Extension Points](/guide/cluster-extension-points) (identity labels, budget guard, cluster events, deterministic IDs/replay, loop checkpoints, retention caps).
...
Rust hosts inject the trait directly. Node.js, Python, and Go expose callback
...
`LoopCheckpoint` after each tool result that lands on the fact log, keyed
by `run_id`. Any node holding the same store can recover the run: the
checkpoint seeds a fresh fact-log thread, and folding that log chooses the
next step. The checkpoint never chooses the next model call itself.
...
The resumed work is recorded as a **new** run; the checkpoint's run
is not modified. Without a checkpoint, `resumeRun` folds the session's
existing fact log when it has facts (a quiescent log takes no step). The
two errors below fire only when the log is also empty:

- `"resume_run requires a session_store on this session"`: host should
  fall back to a fresh session.
- `"no loop checkpoint found for run 'X'"`: the run never wrote a
  checkpoint, or the checkpoint was deleted when the run ended.
...
`now - threshold_ms`. The server's _registered config_ stays — a
...
Missing methods on the guard object are treated as a permissive default
(Allow / no-op). Python and Go callback errors, timeouts, and malformed
decisions fail closed as Deny.
...
arguments; exceptions fail closed. Go handlers receive typed contexts;
