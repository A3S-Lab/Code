...
`a3s code` terminal app, see [A3S Code TUI](/guide/tui#slash-commands).

The two surfaces are intentionally different:

| Surface                   | Owner            | Purpose                                                                                                                   |
| ------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `a3s code` slash commands | CLI/TUI          | Terminal controls such as `/model`, `/effort`, `/memory`, `/kb`, `/update`, and `/exit`.                                  |
...
| Redirect the active run           | `session.steer(text)`                                                                 |
| Stop the active run               | `session.interrupt()`                                                                 |
| Answer a pending confirmation     | `session.confirmToolUse(toolId, approved)`                                            |
