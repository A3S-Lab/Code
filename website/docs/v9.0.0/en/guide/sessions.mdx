...
Each session has one fact log at `<workspace>/.a3s/effect-log/<thread>.jsonl`.
A `send` or `stream` appends the prompt as a `user.message` fact and folds the
log to choose each next step: call the model, run a tool, compact, park, or
finish. A run takes at most 32 steps before it stops with a `StepLimit` error.
See [Fact-log control](/guide/architecture#fact-log-control) for the full model.

What this means for a host:

- **Confirmations park.** A tool call that needs approval emits
  `confirmation_required` and waits. Answer it with `confirmToolUse` (Rust and
  Python `confirm_tool_use`, Go `ConfirmToolUse`).
  The answer is recorded as a fact, so it also works after a restart. The log
  has no timeout of its own.
- **Questions park.** An `ask_user` call emits `user_question` and waits for
  the answer.
- **Tool work is bounded per turn.** The stock tool budget ends a turn after 8
  successful tool results. The tool-round cap (`maxToolRounds`, Python
  `max_tool_rounds`, Go `MaxToolRounds`) makes the next model call see no tools,
  so it must answer in text.
- **Steer is a fact.** An applied steer is another `user.message`, so it also
...
const stream = await session.stream(
  'Run the focused tests and explain failures',
);
...
```python
for event in session.stream("Run the focused tests and explain failures"):
...
## Steer and Interrupt an Active Run

Run control changes an operation already in progress; it does not start a
second transcript turn. `steer` queues a newer user direction; at the next
provider call it is appended to the fact log as a `user.message` and sent with
that request. `interrupt` cancels current provider and Tool work cooperatively,
then lets supervised cleanup settle before the Run becomes `cancelled`.
...
let receipt = session
    .steer(SteerRequest::new("Prioritize the failing test"))
    .await?;
println!("{:?}", receipt.state);

session
    .interrupt(InterruptRequest::new().with_reason("User stopped the run"))
    .await?;
```
...
const receipt = await session.steer('Prioritize the failing test', {
  runId: state?.runId,
...
if state != nil {
	...
	options.ExpectedTurnID = state.TurnID
	options.ExpectedTurnRevision = &state.TurnRevision
}
...
_, err = session.Interrupt(ctx, &code.InterruptOptions{})
```

</Tab>
</Tabs>

Requests are idempotent when a caller reuses the same request ID with the same
payload. Optional Run and expected-Turn fields make stale UI actions fail
closed. Accepted control is not yet applied control: observe
`run_control_applied` or inspect persisted Run events when the distinction
matters. Neither operation changes the selected model, permissions, sandbox,
budgets, or approval policy.
...
`session.resumeRun(runId)` instead. Go uses `Save`, `ResumeSession`, and
`ResumeRun(ctx, runID)` for the same two persistence levels.
See [Persistence](/guide/persistence).

With a checkpoint for that run, `resumeRun` rebuilds the session's fact log
from the checkpoint transcript. Prior model turns are recorded as facts and are
not sent to the model again. Without a checkpoint, it folds the existing log.
Either way the fold chooses the next step: a tool call whose result is missing
runs once, and a quiescent log resumes with zero steps. With neither a
checkpoint nor a log, it fails and names the missing session store or
checkpoint.
...
(Python) / `session.IsClosed(ctx)` (Go).
...
| Node (camelCase)  | Python (snake_case) | Go                | Meaning                                                       |
| ----------------- | ------------------- | ----------------- | ------------------------------------------------------------- |
| `tenantId`        | `tenant_id`         | `TenantID`        | Multi-tenant label                                            |
| `principal`       | `principal`         | `Principal`       | User / service that triggered the session                     |
| `agentTemplateId` | `agent_template_id` | `AgentTemplateID` | Agent template / definition the session was instantiated from |
| `correlationId`   | `correlation_id`    | `CorrelationID`   | Distributed-trace correlation id                              |
...
For disposable value-defined workers, use `SessionForWorker`; both methods
return the regular Go `Session` API.
