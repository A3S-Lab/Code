...
Python guards may also define `check_before_tool(session_id, tool_name)`.
`session.set_budget_guard(guard, timeout_ms=5000)` installs a guard on a live
session. Exceptions, malformed decisions, and timeouts fail closed.
...
A **missing guard method** is treated as the permissive default in every SDK. Callback errors, timeouts, and malformed `check*` returns fail closed as `deny` in Python and Go; Node callbacks must not throw, and hung or unreadable Node `check*` callbacks also fail closed.
...
With a `sessionStore` / `session_store` configured, the agent loop persists a loop checkpoint after **each tool result** that lands on the fact log, keyed by run id. Any node that shares the same store can recover the run. The checkpoint supplies facts, not decisions: the recovering node seeds a fresh fact-log thread from it and folds that log, which chooses the next step.
...
A tool call whose result is missing from the log runs once on resume. Hosts that need an exact, replay-safe recovery id use `spawnRecoveryWithRunId`. See [Persistence](/guide/persistence) for the full rules.
