...
The perception, memory, planning, reasoning, rate-limit, confirmation, success,
and intent events serve specialized harnesses. Registering one is accepted
everywhere, but it fires only on runtime paths that emit it. Treat an
