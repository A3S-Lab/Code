...
A3S Code runs multi-agent orchestration as a _grammar_ expressed in code, then
places the resulting steps wherever you want them to run. The split is drawn
along the **framework / host boundary**:
...
- `AgentStepSpec` — _what_ to run, independent of _where_: `task_id`, `agent`,
  `description`, `prompt`, and optional `max_steps`, `parent_session_id`,
  `output_schema`.
- `StepOutcome` — the result of running one spec: `task_id`, `session_id`,
  `agent`, `output`, `success`, optional `structured`, and `source_anchors`
...
Go exposes the same grammar as `session.Parallel(ctx, specs, budgetTokens)`,
`session.Pipeline(ctx, items, stages)`, and
`session.ParallelResumable(ctx, specs, workflowID)`:
...
executor — the framework migrates the _what_, the host supplies the _where_.
...
See [Orchestration](/guide/orchestration) for the combinator grammar in
depth and [Persistence](/guide/persistence) for the checkpoint store.
