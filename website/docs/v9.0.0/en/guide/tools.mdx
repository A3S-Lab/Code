...
| Code intelligence        | `code_symbols`, `code_navigation`, `code_diagnostics`                       | Registered only when the workspace carries a code-intelligence provider (`WorkspaceServicesBuilder::code_intelligence`). The default local backend has none.                                                       |
| Runtime builtins         | `web_fetch`, `web_search`, `update_plan`, `ask_user`, `batch`, `program`    | Registered by the core tool executor. `batch` and `program` receive the current scoped invoker, so their usable inner tools depend on the session surface and keep the caller's governance scope.                  |
| Session bootstrap tools  | `task`, `generate_object`, `search_skills`, `Skill`                         | Added while building an `AgentSession`. `task` is the only model-visible delegation schema; multi-item fan-out goes through `task`. Delegation disappears when manual delegation is disabled.                      |
...
A presentation profile (`a3s.code.tool-presentation-profile.v1`,
`core/src/tools/presentation.rs`) decides which tool definitions the model sees
for a prompt. It changes presentation only; permission policy, confirmation,
and the sandbox still govern every call.

| Mode       | Model-facing definitions                                               |
| ---------- | ---------------------------------------------------------------------- |
| `adaptive` | Default. A prompt-sensitive selector presents a relevant subset.       |
| `direct`   | Every definition that permission visibility admits.                    |
| `code`     | `program` as a compact code gateway to the other tools.                |
| `disabled` | No tools. Host-direct APIs such as `session.tool(...)` are unaffected. |
...
let options = SessionOptions::new()
    .with_tool_presentation_profile(ToolPresentationProfileV1::direct());
```
...
```ts
import { ToolPresentationMode } from '@a3s-lab/code';

const session = agent.session('/repo', {
  toolPresentationProfile: {
    schema: 'a3s.code.tool-presentation-profile.v1',
    mode: ToolPresentationMode.Direct,
  },
});
console.log(session.toolPresentationProfile());
```
...
opts = SessionOptions()
opts.tool_presentation_profile = ToolPresentationProfile.direct()
session = agent.session("/repo", opts)
print(session.tool_presentation_profile())
```
...
session, err := agent.Session(ctx, "/repo", &code.SessionOptions{
	ToolPresentationProfile: code.NewToolPresentationProfile(code.ToolPresentationDirect),
})
if err != nil {
	log.Fatal(err)
}
profile, err := session.ToolPresentationProfile(ctx)
```
...
| Runtime internal           | `dynamic_workflow` starts its private `program` execution engine                                               | Skips only the duplicate permission/HITL decision already made for `dynamic_workflow`; hooks, evidence, budgets, cancellation, recursion checks, and all script Tool calls remain governed. |
...
The policy is stored in the session snapshot. Resume inherits it when omitted
and rejects an explicitly different policy, so replay cannot silently change
...
When `read` opens a JPEG, PNG, GIF, or WebP file, the tool returns an image
`Attachment` instead of pretending the bytes are text. That matches the tool
description. On OpenAI-compatible providers, tool-result messages keep
multimodal `image_url` parts rather than flattening those attachments to
placeholder text (#156 / #152).
...
separate planes: `grep` never opens durable a3s-vec FTS, and `bm25` never becomes
...
Semantic and hybrid calls use the same tool rather than a separate vector
database tool:
...
Index construction is asynchronous and file-atomic. Results are reread and
digest-verified against current source before rendering, and closing the
session releases every vector. See
[Workspace Retrieval](/guide/context#workspace-retrieval) for activation,
partial-readiness behavior, embedding routes, resource bounds, and lifecycle.
...
path executes configured native API engines first, conventional HTTP/RSS
engines only when the combined structural retrieval requirements are not met,
and headless engines only when both earlier tiers remain insufficient
(`core/src/tools/builtin/web_search/fallback.rs`). `cascade_order` overrides
the order. The headless tier exists only in builds with the `headless-search`
feature (included in `scientific` and `full`, not in the Core default), and
browser discovery and pool creation are lazy. Moli-backed headless engines
arrived with `a3s-search` v3.1.0; A3S Code 9.0.0 pins `a3s-search` 3.1.4 and
uses the packaged or shared-cache Moli runtime when the headless tier runs.
Chrome and Lightpanda remain explicit configured backends.
...
The structural gate only decides whether the cascade continues to the next
tier. Default order is API, then HTTP/RSS, then headless. Usable rows are a
successful tool result: `complete` when the gate is met and `partial` when it
is not. JSON output keeps the result-array contract in both cases. Empty
results, unknown engines, and invalid arguments stay errors. Search does not
use an external semantic verifier or reranker.
...
`web_fetch`, `download`, and `web_search` choose one proxy in this order: the
`search.headless.proxy_url` setting, then `HTTPS_PROXY`, `https_proxy`,
`HTTP_PROXY`, or `http_proxy`, then the macOS system proxy. A per-call
`web_search` `proxy` argument takes precedence over all of these. When a proxy is
selected, every request from these tools goes through it and `NO_PROXY` is not
consulted (`core/src/tools/builtin/safe_http.rs`); unset the proxy variables
for the process if a destination must be reached directly. Without a proxy,
these tools connect directly to validated public addresses. The `NO_PROXY`
handling in `core/src/llm/http.rs` applies to model provider, MCP HTTP, and
OAuth clients, not to the web tools.
...
`program` and `dynamic_workflow` are kept out of the default TUI PTC
allow-list. QuickJS may call `task` with one item, but
...
A report from `session.verifyCommands(...)` is host evidence. It is not bound
to a turn's mutation effect digest, so it does not close the completion gate;
see [Verification](/guide/verification#the-completion-gate).
