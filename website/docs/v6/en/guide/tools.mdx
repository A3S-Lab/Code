...
Without an explicit `engines` argument, `web_search` runs a lazy,
quality-gated cascade: native APIs first, conventional HTTP/RSS engines only
when the combined result set is insufficient, and headless engines only when
the earlier tiers still miss the generic quality floor. The floor measures
usable results, host diversity, provenance, and language-neutral query
coverage; it contains no topic, publisher, domain, or language routing rules.
An explicit engine list executes only the requested tiers.

Search metadata includes `search_tiers`, `search_quality`, `engine_outcomes`,
typed `engine_failures`, and bounded latency/error metrics. Circuit state is
session-scoped, while per-engine concurrency limits and headless retry budgets
are shared across sessions created by one `Agent` and inherited by delegated
children. Identical overlapping requests are coalesced within a session and
its delegated children; completed flights are removed immediately rather than
cached. `search_coalescing` metadata exposes current capacity plus leader,
shared, bypassed, and abandoned request counts. Chrome or Lightpanda discovery
happens only if the headless tier is actually reached; browser installation
and repair remain explicit host operations.

If every available tier is exhausted without meeting the quality floor,
`web_search` returns a tool failure instead of presenting weak candidates as a
successful evidence set. JSON mode uses a `quality_not_met` diagnostic envelope
containing the observed quality, the floor, and bounded candidates so an agent
can reformulate the query without silently citing them.
