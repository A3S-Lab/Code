...
through `errors.Is`. Rust trait-object injections such as a custom `LlmClient`,
Workspace backend, hook executor, or memory implementation remain Rust-only;
the bridge accepts their serializable configuration counterparts where one
exists.
...
The shared guides now place Go beside Node.js and Python where the bridge
exposes the same capability. Start with
