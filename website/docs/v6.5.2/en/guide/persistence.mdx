...
The current Go bridge exposes snapshot `Save` / `ResumeSession`, run records,
and replay events, but not the `resumeRun` loop-checkpoint operation or workflow
checkpoint orchestration. This guide shows a Go tab only where that operation
is actually part of the Go surface.
