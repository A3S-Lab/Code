...
`session.resumeRun(runId)` instead. The Go bridge supports snapshot
`Save`/`ResumeSession`, but does not expose interrupted-run `resumeRun`.
...
The Go bridge does not expose `sessionForAgent`. Configure `AgentDirs` in
`SessionOptions`, then create the session through `Agent.Session`; use the
regular session API for the resulting conversation.
