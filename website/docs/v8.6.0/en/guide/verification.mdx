...
on its own. Since 8.5.6, a workspace mutation cannot complete the turn unless a
Passed verification report is bound to that mutation digest, or a host waiver
covers that digest. A host waiver is not model-grantable. Verification turns
the claim into evidence: you declare commands that _must_ succeed, the runtime
executes them, and the result carries a report you can inspect, gate on, or
surface to a user.
