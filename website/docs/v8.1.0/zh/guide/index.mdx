...
8.1.0 让四种 SDK 都可以发现完整的产品能力清单，并集成
`a3s-search` v3.1.0；默认的 JavaScript 网页搜索后端改为 Moli。支持的发布
目标会携带匹配的 Moli sidecar；同一用户下的多个 A3S Code 进程会复用经过校验
的共享缓存，不会为每个应用重复安装浏览器。
...
v8.1.0 的 Python 包通过每个平台一个 `cp310-abi3` Wheel 支持 CPython 3.10 至
3.14。Intel Mac 使用 `macosx_12_0_x86_64` Wheel，最低需要 macOS 12。请参阅
[Python Wheel 平台](/api/#python-wheel-平台-v810)了解 Bootstrap 流程，以及
