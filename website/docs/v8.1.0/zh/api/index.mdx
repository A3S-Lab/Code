...
## Python Wheel 平台（v8.1.0）

PyPI 上的 `a3s-code` 是一个纯 Python Bootstrap。首次导入时，它会从 v8.1.0
...
`sdk_capabilities()`（或对应语言名称）都会返回相同顺序的 26 条能力记录。
...
通过后才可能下载。v8.1.0 的上游 Moli 没有 Linux musl 资产；该平台请提供系统/显式
