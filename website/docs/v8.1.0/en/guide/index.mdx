...
Version 8.1.0 makes the complete product capability inventory discoverable from
all four SDKs and promotes `a3s-search` v3.1.0 with Moli as the default
JavaScript-capable search backend. Release packages carry the matching Moli
sidecar for supported targets; processes that share a user account reuse one
verified cache rather than installing a browser per application.
...
The v8.1.0 Python package supports CPython 3.10–3.14 through one
`cp310-abi3` wheel per platform. Intel Macs use the `macosx_12_0_x86_64` wheel
and require macOS 12 or newer. See [Python wheel platforms](/api/#python-wheel-platforms-v810)
