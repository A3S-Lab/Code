...
Each active Run owns a bounded control inbox. `steer` appends a new user
direction to the same transcript at the next provider or tool safe point;
`interrupt` requests cooperative cancellation and lets normal cleanup,
checkpointing, hooks, and event persistence finish. Idempotency keys, immutable
