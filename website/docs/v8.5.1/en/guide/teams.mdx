...
Teams are a harness pattern built from named agent definitions plus the unified
`task` delegation core. A3S Code does not expose a separate team runner API; the
parent session stays responsible for synthesis, policy, and final verification.
...
`session.tasks(...)` is the host-driven multi-item wrapper around `task`; it
returns a `ToolResult`, not a `StepOutcome[]`. Use `session.parallel(...)` when
you need one structured outcome per lane.

When the team's lane structure is fixed and should be reproducible and resumable rather than model-chosen, use the programmable combinators in [Orchestration](/guide/orchestration) (`session.parallel` / `session.pipeline` / `session.parallelResumable`).
