...
The Code TUI uses the A3S-owned `a3s-sandbox` Rust library through its
`NativeBashSandbox` adapter. It selects the native boundary for the host
...
bounded command through the actual OS boundary before attaching it. Failure
leaves the sandbox absent, so Default can request one exact host invocation and
Auto denies Bash.
