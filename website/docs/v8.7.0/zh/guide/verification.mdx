...
运行时将"完成"视为必须被**证明**的事实，而不仅仅是被声明的结果。当模型说某个任务已完成时，这句话本身毫无价值。自 8.5.6 起，工作区变更不能完成这一轮，除非有一份 Passed 的验证报告绑定该变更 digest，或宿主豁免覆盖该 digest。宿主豁免不能由模型授予。验证把声明转化为证据：你声明一组*必须*成功的命令，运行时执行它们，结果会携带一份你可以检视、据以拦截或呈现给用户的报告。
