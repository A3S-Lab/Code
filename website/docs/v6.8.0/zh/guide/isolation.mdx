...
文件、搜索、shell 和 git 操作从 session 工作区求值。`task` 与自动 subagent 委派会隔离子运行上下文，父 agent 只合并紧凑结果。Security provider、hooks 和 permission policy 是集成点；把它们作为生产边界前，应对实际策略路径做 harness 测试。
