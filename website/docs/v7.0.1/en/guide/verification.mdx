...
evidence and keeps unresolved gaps visible. CI rejects a capability-map change
that is not reflected in that ledger. Node.js and Python gates build and load
their native modules before exercising the public wrappers; compile-only Rust
checks are not counted as SDK runtime evidence. Go runs through its versioned
...
for the current evidence ledger, known gaps, qualification commands, and the
completion rule. A repository-wide claim is not complete while that ledger
contains an unresolved Code-owned gap.
