...
| Tools and safety         | File, binary-safe `download`, search, shell, git, web, structured-output, MCP, PTC `program`, `task`, and `parallel_task` tools pass through workspace boundaries, permission policy, HITL approval, timeouts, hooks, and traces.                                                                                                                           |
...
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Workspace tools   | `read`, `write`, `edit`, `patch`, local `download`, `ls`, `glob`, `grep`, `bash`, `git`, `web_fetch`, and `web_search` run through workspace services, path boundaries, timeout handling, sandbox enforcement, and confirmation policy. |
