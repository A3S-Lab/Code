...
| ------------------------ | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Workspace-gated builtins | `read`, `write`, `edit`, `patch`, `download`, `grep`, `glob`, `ls`, `bash`, `git` | Registered only when `WorkspaceServices` advertises the required capability. `download` additionally requires a writable local workspace; remote backends do not expose it.                                |
...
### Quality-gated web search

`web_search` reports `complete`, `partial`, or `failed` in metadata. Its default
path executes native APIs first, conventional HTTP/RSS engines only when the
combined generic quality floor is not met, and headless engines only when both
earlier tiers remain insufficient. Browser discovery and pool creation are
...
The final cascade fails closed when it still does not satisfy the generic
quality floor. Successful JSON output keeps the result-array contract.
Insufficient JSON output is an error with a typed `quality_not_met` envelope,
the candidate rows, measured quality, and required floor. Every result row also
includes `query_match_score`.
...
rate-limit errors carry structured error kinds, while tier decisions, quality
signals, engine outcomes, attempt duration, and retry context remain available
in metadata.
