...
搜索使用质量门控级联：原生 API、HTTP/RSS 引擎，以及 Core 默认 feature 中惰性启动的
Chrome/Chromium 层。完整级联仍低于通用质量下限时会失败关闭，不会把弱候选当成成功
证据。委派上下文共享 bulkhead、重试预算和相同请求合并，详见
[工具](/guide/tools#质量门控的网页搜索)。
