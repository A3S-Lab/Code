...
| 层级                       | 工具                                                                              | 注册规则                                                                                                                                 |
| -------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 受工作区能力限制的内置工具 | `read`、`write`、`edit`、`patch`、`download`、`grep`、`glob`、`ls`、`bash`、`git` | 仅在 `WorkspaceServices` 声明具备所需能力时注册；`download` 还要求可写的本地工作区，远端后端不会暴露它。                                 |
...
如果完整级联仍未达到通用质量下限，最终结果会失败关闭。成功的 JSON 输出保持结果数组
契约；质量不足的 JSON 输出是错误，并携带类型化 `quality_not_met` envelope、候选行、
实测质量和要求下限。每条结果还会包含 `query_match_score`。
