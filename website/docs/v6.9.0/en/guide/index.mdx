...
- Agent sessions now share one `a3s-lane` priority/FIFO admission boundary.
  Non-urgent work ages toward interactive priority, and Rust, Node.js, Python,
  and Go expose the same per-session priority and scheduler occupancy data.
- Personal and project `AGENTS.md` instructions form one bounded, provenance-
  preserving chain. Lifecycle hooks can govern prompt rewrites, permissions,
  compaction, and session start/end without bypassing schema validation.
- Native Agent Harness conversations can use isolated Git worktrees and emit
  immutable SHA-256-bound patches through the bounded changes protocol.
- Tool-result transforms now retain deterministic evidence, including source
  and projected digests, byte/token deltas, loss mode, and immutable artifact
  references. Cognitive-package sessions retain and verify their exact A3S Use
  generation binding across persistence and replay.
