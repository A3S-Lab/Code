...
- Agent Session 现在共享一个基于 `a3s-lane` 的优先级/FIFO 准入边界。非 urgent
  工作会逐级老化到 interactive；Rust、Node.js、Python 和 Go 提供相同的
  Session 优先级与调度器占用数据。
- 个人与项目 `AGENTS.md` 组成一条有界且保留来源的指令链。生命周期 Hook 可以治理
  Prompt 重写、权限、上下文压缩和 Session 启停，同时重写后的工具参数仍会再次通过
  Schema 校验。
- 原生 Agent Harness 对话可以使用隔离 Git worktree，并通过有界 changes 协议输出
  绑定 SHA-256 的不可变 Patch。
- Tool result 变换会保留确定性证据，包括源/投影摘要、字节与 Token 差值、损失模式和
  不可变 Artifact 引用。Cognitive-package Session 也会跨持久化与 Replay 保留并校验
  精确的 A3S Use generation binding。
