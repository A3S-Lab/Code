...
async fn main() {
    ...
    let base_url =
        std::env::var("KIMI_BASE_URL").expect("KIMI_BASE_URL environment variable not set");
    ...
    let agent = match Agent::create("examples/agent_kimi.acl").await {
        Ok(a) => a,
        Err(e) => {
    ...
    println!("📝 Creating session with file-reader skill and explicit permissions...");
    let workspace = std::path::PathBuf::from("examples");
    let permission_policy = PermissionPolicy::new().allow("Skill(*)");
