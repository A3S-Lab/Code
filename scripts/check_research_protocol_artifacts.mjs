...
/** Cross-language parity check for generated research wire declarations. */
...
const manifest = JSON.parse(read('sdk/research/research-wire-v1.json'));
const fixtures = JSON.parse(read('sdk/research/research-wire-v1-fixtures.json'));
assert.equal(manifest.schema, 'a3s.code.research-wire.v1');
assert.equal(manifest.version, 1);
assert.equal(manifest.max_message_bytes, 32 * 1024 * 1024);
assert.equal(manifest.kinds.length, 13);
...
assert.equal(fixtures.valid.kind, 'research_event');
...
const node = read('sdk/node/research-protocol-v1.d.ts');
const python = read('sdk/python/python/a3s_code/research_protocol_v1.py');
const go = read('sdk/go/research_protocol_v1.go');

for (const source of [node, python, go]) {
  assert.match(source, /a3s\.code\.research-wire\.v1/);
...
console.log(`research protocol SDK parity aligned (${names.length} kinds)`);
