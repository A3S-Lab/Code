...
async def evaluate(
    ...
    model, dimension, model_load_ms = await asyncio.to_thread(
        load_sentence_transformer, model_id, revision, local_files_only
    )
    ...
    provider = sentence_transformer_provider(
        ...
        dimension,
        counters,
        query_id=FIXTURE["embedding"]["query_id"],
    ...
    with tempfile.TemporaryDirectory(prefix="a3s-real-embedding-eval-") as root_text:
        ...
        try:
            ...
            closed = cast(dict[str, Any], session.workspace_retrieval_status())
