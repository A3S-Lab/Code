...
fn model_middleware_health_fixture_is_secret_free_default() {
    ...
    let encoded = serde_json::json!({
        "trustAdmitted": health.trust_admitted,
        "trustRejected": health.trust_rejected,
        "providerCalls": health.provider_calls,
        "usageRecorded": health.usage_recorded,
        "completionCalls": health.completion_calls,
        "streamingCalls": health.streaming_calls,
        "trustedToolResults": health.trusted_tool_results,
        "workspaceDataToolResults": health.workspace_data_tool_results,
        "externalToolResults": health.external_tool_results,
        "externalRedactionReviewed": health.external_redaction_reviewed,
    });
    assert_node_middleware_health_fixture(&encoded, &fixture);
}
...
fn sdk_capability_batch_fixture_commits_skill_generation() {
    ...
    let batch = fixture["sample_batch"].clone();
    for field in fixture["required_batch_fields"]
        .as_array()
        .expect("required batch fields")
    {
        assert!(batch.get(field.as_str().unwrap()).is_some());
    }

    let session = build_test_session();
    let receipt = fallback_runtime()
        .expect("test runtime builds")
        .block_on(session.apply_capability_batch(batch))
        ...
        .expect("required receipt fields")
    {
        ...
        assert!(receipt.get(name).is_some(), "missing receipt field {name}");
    }
    assert_no_forbidden_middleware_fields(&receipt, &fixture);
    let stamp = session
