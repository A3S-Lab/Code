// Real-LLM validation of PTC fan-out through `task` multi-item calls.
//
// A `program` script calls `task` with N identical tasks. We time the
// program tool (its only work is that one task call). Parallel fan-out
...
function script(n) {
  ...
  const res = await ctx.tool("task", { tasks: ${JSON.stringify(tasks)} });
...
async function run(n) {
  ...
  while (true) {
    ...
    if (ev.type === 'tool_end' && ev.toolName === 'program') {
      ...
      ok = /exit_code=0/.test(programOutput) && /task \(ok/.test(programOutput);
    }
...
console.log(`\nSUMMARY`);
console.log(`  pass: ${good.length}/${SOAK} (program ran task fan-out ok + all 3 results)`);
