 ...
 * Uses the config from a3s/.a3s/config.acl
 *
 * This test will:
...
const { Agent, Orchestrator } = a3sCode;
...
async function main() {
  ...
  // Get config path. This is a real-provider example and is skipped when no
  // local config is available.
  const configPath = process.env.A3S_CONFIG ||
    path.join(__dirname, '..', '..', '..', '..', '..', '.a3s', 'config.acl');
  if (!fs.existsSync(configPath)) {
    console.log(`Kimi orchestrator example skipped; config file not found: ${configPath}`);
    console.log('Set A3S_CONFIG to run this real-provider example.');
    process.exit(0);
  }
