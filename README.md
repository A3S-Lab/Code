...
- **Windows test isolation (8.5.8).** Credential-boundary tests do not inherit
  ...
  `\\?\` paths.
- **Native Bash sandbox Gate 7 (8.5.10).** Default Bash profile stays network
...
| Typed System-1 decisions | In-process Apofasi System One (`choice` / `score` / `noul`) with host `GatePolicy` (`auto` / `escalate`), `TypedDecisionEngine` trait, and digest-bound `TypedDecisionReceiptV1`; lexical by default, optional neural checkpoints. When `apofasi` is enabled, Code refuses to replace planning pre-analysis and goal achievement, because each generation returns more than one typed answer. Those call sites refuse to skip. Code does not add a keyword classifier and does not lower `GatePolicy`. The escalate prompt includes the task state. The system prompt is not rewritten | Cargo features `apofasi`, `apofasi-infer`, `apofasi-metal` (opt-in; not in `local-code` / `scientific` / `full`); Advanced inventory id `typed_decisions`; extra call sites stay host-owned; not a generative model path and not a Use-projected capability kind yet |
