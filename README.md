...
- [x] **Graceful degradation on lock poisoning in security module**
  - Replaced all `.unwrap()` on `RwLock` in security module with `let Ok(..) = .. else { return }` graceful fallback
  - Security hooks fail-safe (interceptor blocks, sanitizer passes through) on poisoned locks

- [x] **Sanitize session IDs — block path traversal**
  - Added `validate_path_safe_id()` — rejects IDs with `..`, leading `.`, or non-ASCII-alphanumeric characters
  - Applied to `Session::new()` and `CheckpointManager::checkpoint_dir()`

- [x] **Tool argument parse failure must error, not silently degrade**
  - Malformed tool call JSON now produces `__parse_error` marker instead of empty `{}`
  - Agent loop detects marker and returns error to LLM for retry

**P1 — Critical Attack Surface** ✅

- [x] **Scan tool outputs for prompt injection (indirect injection defense)**
  - Added `ToolOutputInjectionScanner` as `PostToolUse` hook — scans high-risk tool outputs (`read`, `web_fetch`, `bash`)
  - Logs detection to audit log without blocking (avoids false positives on legitimate code)

- [x] **Builtin tool shadowing prevention**
  - Added `register_builtin()` to `ToolRegistry` — builtin tools tracked in a `HashSet`
  - `register()` rejects tools whose name collides with a builtin

- [x] **Workspace boundary enforcement in-process**
  - Added `check_workspace_boundary()` in `ToolExecutor` — validates file paths resolve within workspace
  - Resolves symlinks via `canonicalize()` before prefix check
  - Applied to: read, write, edit, patch, glob, ls, grep

- [x] **API keys: `SecretString` wrapper with redacted Debug/Display**
  - `AnthropicClient.api_key`, `OpenAiClient.api_key`, `LlmConfig.api_key` now use `SecretString`
  - `Debug` and `Display` output `[REDACTED]` — prevents accidental key leakage in logs

**P2 — Defense in Depth** ✅

- [x] **MCP/script tool execution timeout**
  - Added configurable `tool_timeout_secs` to `McpServerConfig` (default: 60s)
  - `StdioTransport` enforces per-request timeout, killing stalled MCP calls
  - Proto field added for SDK configuration

- [x] **HITL timeout default → Reject**
  - `TimeoutAction::Reject` is already the `#[default]` — timeout means refusal, not consent
  - `AutoApprove` available as explicit opt-in via `ConfirmationPolicy`

- [x] **Persistence failure visibility**
  - Added `AgentEvent::PersistenceFailed` variant with session_id, operation, and error
