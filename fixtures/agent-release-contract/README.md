...
4. bind that digest and the exact binary/BuildKit provenance through
   `AgentReleaseManifest::bind_publication`; and
...
`release-output/.a3s/asset.acl`
plus a secret-free `publication.json`. Deploy only the
`exactImageReference` recorded there, never the mutable input tag. The supplied
binary is represented by a digest-bound `source` entry. The recipe, Buildx
version, Cargo/Rust versions, publication source, and canonical build metadata
are covered by the `builder` digest; secret values are never script inputs.
That provenance binds the build that occurred. It does not claim byte-for-byte
reproducibility from Ubuntu package repositories at a later date.
...
liveness, value-redacting protocol errors, SIGTERM shutdown, exit status, and
