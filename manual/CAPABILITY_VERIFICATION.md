...
For the dated 29-id effective / efficient / integrated-use snapshot (bailian
Flash L5), see
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| Agent runtime           | [`agent_api` tests](../core/src/agent_api/tests/mod.rs), [`run_lifecycle`](../core/src/agent_api/run_lifecycle.rs), and [`test_session_close_lifecycle`](../core/tests/test_session_close_lifecycle.rs) cover construction, send/stream admission, resume, replace, cancel, close, and replay.                                                                                                         | [`agent_convergence_benchmark`](../core/examples/agent_convergence_benchmark.rs) executes scripted end-to-end convergence and checkpoint recovery.                                                                                                       | Tool rounds, the empty-tool-list cap, provider calls, token accounting, close deadlines, and single-flight admission are bounded. A narrative turn is returned without continuation injection. The convergence report records latency but gates deterministic work amplification rather than noisy absolute milliseconds. | Required CI plus targeted release qualification.                         |
...
| Evaluation substrate    | [`evaluation substrate tests`](../core/tests/evaluation_substrate.rs) and [`qualification tests`](../core/tests/evaluation_qualification.rs) cover digest-only facts, bounded evidence, isolated auxiliary lifecycle, cancellation, replay, redaction, strict wire/result validation, durable restart, retention, and lease fencing. | The provider-free qualification suite exercises independent file-backed stores and supervisor restart/retry behavior; host policies, executors, authorization, and business result vocabularies remain outside Core. | [`evaluation_substrate_benchmark`](../core/examples/evaluation_substrate_benchmark.rs) gates bounded result publication, reopen validation, wire round trips, persisted bytes, file count, and p50/p95 resource ceilings. Hosted [performance run 33844533910](https://github.com/A3S-Lab/Code/actions/runs/33844533910) contains all nine passing machine-readable profiles; hosted [CI run 33847689080](https://github.com/A3S-Lab/Code/actions/runs/33847689080) passes the strict release gate. | Required correctness CI and hosted release qualification; provider, network, Cloud, and product evaluator evidence remains external. |
| Typed System-1 decisions | [`typed_decision` tests](../core/src/typed_decision.rs) cover lexical `choice` / `score` / `noul`, fail-closed empty requests, host `GatePolicy` escalate thresholds, digest-bound `TypedDecisionReceiptV1`, route admission, `TypedDecisionEngine` trait shape, and neural loader feature gating. | Host product routing, call sites, and threshold policy remain outside Core; neural checkpoint weights and Metal/CUDA qualification are host-owned. Use-projected `CapabilityKind` is not claimed. | Lexical path has no ML framework and no weight download. Receipt digests are domain-separated SHA-256 over request/response JSON. Neural path stays behind `apofasi-infer` / `apofasi-metal`. | Required CI for the Code substrate boundary; Desktop/CLI/Cloud product wiring and neural performance remain external. |
| Native research contracts | [`research` module tests](../core/src/research), [`research_execution_qualification`](../core/tests/research_execution_qualification.rs), and [`research_review_qualification`](../core/tests/research_review_qualification.rs) cover versioned run/evidence/claim/citation/graph/workflow/repro/provenance/review/wire values, fail-closed digests, lifecycle transitions, and host-injected evaluator composition. | [Research Contracts](RESEARCH_CONTRACTS.md) documents ownership; scientific policy, rubrics, retention, and publication remain host/Desktop/Cloud-owned. Generated Node/Python/Go projections under `sdk/research/` stay parity-checked with the Rust catalog. | Wire helpers enforce a 32 MiB bounded decode; digests and identity trees are deterministic. No Core performance claim depends on reviewer model latency. | Required CI for the Code contract boundary; product reviewer and Cloud audit evidence remain external. |
...
| Model adapters          | [`LLM tests`](../core/src/llm/tests.rs), [`OpenAI tests`](../core/src/llm/openai/tests.rs), [`Anthropic adapter`](../core/src/llm/anthropic.rs), [`HTTP admission tests`](../core/src/llm/admission.rs), and [`llm_invoker` middleware tests](../core/src/agent/llm_invoker/tests.rs) cover normalization, streaming, tool calls, errors, retries, redaction, the run-bound trust→budget→evidence→generation→provider→usage pipeline (`OPT-MW1` / `OPT-TRUST1`), and secret-free middleware health counters (`OPT-OBS1`). | [`test_deepseek_adversarial_e2e`](../core/tests/test_deepseek_adversarial_e2e.rs), Codex-login tests, and real-config scripts qualify live transport separately.                                                                                         | Request timeouts, retry limits, response limits, token usage, and cancellation are bounded. Remote latency and cost are reported, not used as deterministic Core speed gates. Middleware health snapshots stay digest-free.                                                                                                                                                  | Required protocol CI plus external qualification.                        |
...
| Priority scheduling     | [`task_scheduler` tests](../core/src/task_scheduler/tests.rs), [`session lane queue` tests](../core/src/session_lane_queue.rs), and [`queue` tests](../core/src/queue.rs) cover priority, FIFO order, aging, occupancy, cancellation, shutdown, digest-only owner quotas, mixed-owner progress, conflicting limits, and idle-state pruning.                                                                                                                                                      | SDK runtime suites verify the exported configuration and snapshots.                                                                                                                                                                                      | Capacity, pending counts, owner limits, aging intervals, admission timeouts, and shutdown are explicit; ordering tests use logical work rather than wall-clock throughput claims.                                                                                                                                                            | Required CI and SDK runtime CI.                                          |
| Safe-point run control  | [`run_control` tests](../core/src/run_control.rs), [`public runtime tests`](../core/tests/run_control_runtime.rs), and Hook/runtime lifecycle tests cover idempotency, immutable Run identity, optimistic turn guards, bounded queues, deadlines, Hook denial/retry, safe-point application, cancellation, close settlement, and monotonic terminal state. | [`test_run_control_real_llm`](../core/tests/test_run_control_real_llm.rs) verifies steering after an in-flight Tool call and interruption without late effects against each configured release model. | Input, reason, ID, retained receipt, and queue sizes are bounded. Steering and interruption reuse the active Run's model, permissions, sandbox, budget, hooks, event stream, and cleanup path; neither opens a concurrent transcript operation. | Required correctness, SDK alignment, and external release qualification. |
...
| Agent release contract  | [`agent_release_manifest`](../core/tests/agent_release_manifest.rs) and [`r0 cross-project contract`](../core/tests/r0_cross_project_contract.rs) cover bounded admission, canonical identity, exact post-build artifact/provenance binding, compatibility, and secret slots. The filesystem-first AgentDir convention and its tests are removed. | The [minimal publication fixture](../fixtures/agent-release-contract/README.md) builds without the final manifest, pushes one OCI image manifest, retains the canonical builder provenance object, generates canonical ACL after digest resolution, verifies the complete digest chain, and can pull/run the exact digest through local Docker. Retained Cloud Runtime and real-provider evidence remain external gates. | Manifest size, file count, canonicalization work, compatibility checks, provenance bytes, health wait, shutdown deadline, and retained secret-free evidence are bounded. | Required CI and release preflight; local Docker plus external certification for deployment claims. |
...
| SDK capability contract | [`SDK capability inventory tests`](../core/src/sdk_capabilities.rs) and the [`SDK API alignment check`](../scripts/sdk_api_alignment_check.mjs) cover the ordered schema, capability metadata, and Rust/Node/Python/Go projections. | [`SDK API design`](SDK_API_DESIGN.md) documents the typed discovery and capability views; each host-language runtime suite loads the public projection. | Capability count, schema version, serialized metadata, and projection ordering are deterministic and bounded; no SDK may silently omit a Core capability. | Required CI and SDK runtime CI. |
| S3 workspace            | [`S3 unit tests`](../core/src/workspace/s3/tests.rs) cover path normalization, capability gating, read/search limits, concurrency, and configuration. [`test_s3_backend`](../core/tests/test_s3_backend.rs) covers live read/write/edit/patch/list behavior.                                                                                                                                       | [`Hermetic Integrations`](../.github/workflows/hermetic-integrations.yml) starts the in-repo S3-compatible fixture (`fixtures/s3-compat`) and executes the live backend test through write, read, list, edit, patch, and cleanup. Official MinIO images are not a dependency. Actual deployment compatibility remains external qualification.    | Read bytes, objects scanned, bytes per object, search concurrency, and request timeout are bounded. The retained S3 report requires the complete roundtrip and zero residual test objects.                                                                                                                                  | Required unit and hermetic object-store integration CI.                  |
...
The v8.2 release qualification composes these capability rows instead of
testing them only in isolation. [`test_extensibility_real_llm`](../core/tests/test_extensibility_real_llm.rs)
drives Skill discovery and invocation, concurrent PTC reads, persisted dynamic
workflow replay, and pre/post Hook behavior. [`test_long_horizon_real_llm`](../core/tests/test_long_horizon_real_llm.rs)
requires planning, a deliberately failing baseline, multi-file inspection and
editing, regression execution, and an independent verification command.
[`test_auto_delegation_real_parallel`](../core/tests/test_auto_delegation_real_parallel.rs)
and [`test_ultracode_discretion_real_llm`](../core/tests/test_ultracode_discretion_real_llm.rs)
separately qualify model-selected SubAgents, built-in agents, parallel fan-out,
and restraint on trivial work. Release evidence must run the complete matrix
against every model declared in the repository-local `.a3s/config.acl`; a pass
from only the default alias is insufficient.
...
a3s-vec lexical projection and the A3S Memory semantic projection. Its exact
...
verified until a host-language process loads the produced engine feature and
...
4. an S3-compatible roundtrip and cleanup gate (in-repo fixture, not a MinIO image);
