...
| Context and memory      | [`context` tests](../core/src/context), [`memory` tests](../core/src/memory.rs), [`durable retrieval evaluation`](../core/tests/durable_memory_retrieval_eval.rs), [`durable product evaluation`](../core/tests/durable_memory_product_eval.rs), [`durable multilingual evaluation`](../core/tests/durable_memory_multilingual_eval.rs), [`durable multi-agent evaluation`](../core/tests/durable_memory_multi_agent_eval.rs), [`durable restart`](../core/tests/durable_memory_restart.rs), [`durable restart endurance`](../core/tests/durable_memory_restart_endurance_eval.rs), and [`maintenance lifecycle`](../core/tests/memory_maintenance_lifecycle.rs) cover ranked assembly, compaction, active-only lexical/relation recall, versioned word/CJK-bigram queries, exact namespaces and resume bindings, pure preview, evidence-backed extraction, real session admission/use, explicit sharing, process-local run-ID reuse, non-replacing collision handling, repeated file-repository reopen, revision history, non-destructive verified supersession, pruning, cancellation, health, lock release, and joined close. | The [retrieval fixture](DURABLE_MEMORY_RETRIEVAL_EVAL.md) locks Recall@5/MRR, safety assertions, and the vector decision. The [product fixture](DURABLE_MEMORY_PRODUCT_EVAL.md) locks no-memory/V1/V2 task success, write precision, evidence fidelity, conflict preservation, context, model-call, nominal-cost, and admission gates. The [multilingual fixture](DURABLE_MEMORY_MULTILINGUAL_EVAL.md) locks the query-profile identity, same-language English/CJK Recall@3 and MRR, real-session context bounds, and Candidate/tenant isolation while explicitly retaining the cross-language miss. The [restart-endurance fixture](DURABLE_MEMORY_RESTART_ENDURANCE_EVAL.md) locks three process epochs, four agents, eight resumes, 24 distinct admissions, one verified revision, and zero stale/Candidate/foreign context under retained run-ID reuse. Real context-tool behavior remains isolated in [`test_context_tools_real_llm`](../core/tests/test_context_tools_real_llm.rs); longer semantic, remote-fencing, provider, latency, cost, and drift distributions remain host qualification. | [`context_memory_benchmark`](../core/examples/context_memory_benchmark.rs) gates ranked/deduplicated assembly over 25,000 inputs, recall over 2,500 memories, output budgets, correctness, and active/retained RSS. Results are retained in the [performance record](PERFORMANCE_QUALIFICATION.md). | Required correctness CI and release qualification. |
