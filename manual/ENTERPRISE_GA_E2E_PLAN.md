...
| L2 F-kernel cov (≥95%) | **PASS** — `/tmp/a3s-llvm-cov-f95/FINAL.txt` `ALL_F_TABLE_KERNELS_GE_95_PASS scored=42`; `core/src/agent_protocol_harness.rs` **95.16%** after SessionClosed→Closed mapper unit test (tip `15d2a863`) |
| L8 §7 9.0.0 fact-log pins | **PASS (hermetic)** — `fact_log` 34/34; effect park + `tool_round_cap`; bm25 persistent a3s-vec FTS 17/17 on tip |
